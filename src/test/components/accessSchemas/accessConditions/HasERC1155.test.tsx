...
import HasERC1155 from '../../../../components/accessSchemas/accessConditions/hasERC1155';
...
const initializeState = (settings:any) => {
  ...
  settings.set(dropUpdatesState, {
    accessSchema: [{
      key: 'hasBalance', chainId: 31337, contractAddress: '0x12345', parameters: ['one', 'two'],
    }],
  });
};

describe('hasERC1155 tests', () => {
  beforeEach(() => {
  ...
  test('It renders', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByDisplayValue('two')).toBeVisible();
  ...
  test('Updates workflow', async () => {
    ...
    mockAccessValidatorInstance.getAccessSchema.mockReturnValueOnce({ mock: 'three' });
    ...
    await userEvent.type(screen.getByTestId('tokenId'), 'three');

    await waitFor(() => {
      expect(mockAccessValidatorInstance.updateAccessCondition).toHaveBeenCalledWith(
        { index: 0, tokenId: 'twothree' },
      );
    ...
    await waitFor(() => {
      ...
      expect(updatesObserver).toHaveBeenCalledWith({ accessSchema: { mock: 'three' } });
    });
  });
});
