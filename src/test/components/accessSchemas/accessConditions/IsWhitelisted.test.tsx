...
import IsWhitelisted from '../../../../components/accessSchemas/accessConditions/isWhitelisted';
...
const initializeState = (settings:any) => {
  ...
  settings.set(dropUpdatesState, {
    accessSchema: [{
      key: 'hasBalance', chainId: 31337, contractAddress: '0x12345', returnValueTest: { value: 'whitelistedAddress' },
    }],
  });
};
...
describe('IsWhitelisted tests', () => {
  ...
  test('It renders', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('WHITELISTED ADDRESS')).toBeVisible();
      expect(screen.getByDisplayValue('whitelistedAddress')).toBeVisible();
    });
  });
  ...
  test('Updates workflow', async () => {
    ...
    await userEvent.type(screen.getByTestId('whitelistedAddress'), 'abc');

    await waitFor(() => {
      expect(mockAccessValidatorInstance.updateAccessCondition).toHaveBeenCalledWith({ index: 0, whitelistedAddress: 'whitelistedAddressabc' });
