...
function IsBlacklisted({ accessOperator }: { accessOperator?:string}) {
  const [jsonSchema, setJsonSchema] = useRecoilState(accessValidatorState);
  const [chainId, setChainId] = useState(1);
  const [blacklistedAddress, setBlacklistedAddress] = useState('');
  ...
  return (
    <Col style={{ border: '1px solid #D5D5D5', margin: '10px', padding: '10px' }}>
      ...
      <Form.Label style={{ display: 'block', margin: '15px 0px 0px 0px' }}>Blacklisted Address</Form.Label>
      <Form.Control
        type="text"
        value={blacklistedAddress}
        onChange={(event) => { setBlacklistedAddress(event.target.value); }}
      />
        ...
        onClick={
            () => {
              validator.addAccessCondition({
                key: 'isBlacklisted', chainId, blacklistedAddress,
              }, accessOperator);
