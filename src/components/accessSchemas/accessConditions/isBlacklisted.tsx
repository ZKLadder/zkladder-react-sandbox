...
function IsBlacklisted({ index }: { index:number }) {
  ...
  return (
    <Row style={{ marginTop: '5px' }}>
      ...
      {/* Blacklisted Address Field */}
      <Col lg={12}>
        <Form.Label className={style['form-label']} style={{ marginTop: '10px' }}>BLACKLISTED ADDRESS</Form.Label>
        <Form.Control
          key="blacklistedAddress"
          className={style['form-input']}
          type="text"
          data-testid="blacklistedAddress"
          ...
          onChange={async (event) => {
            ...
            try {
              ...
              const validator = new AccessValidator(dropUpdates?.accessSchema);
              validator.updateAccessCondition({ index, blacklistedAddress: address });
